...
public class LoggingVisitor implements WorkflowVisitor {
    ...
    @Override
    public void onContinuationStart(String nodeId, String childWorkflowId) {
        logger.log(level, String.format("Node [%s] starting continuation workflow: %s",
            nodeId, childWorkflowId));
    }
    
    @Override
    public void onContinuationComplete(String nodeId, String childWorkflowId, boolean success) {
        logger.log(level, String.format("Node [%s] continuation workflow completed. Success: %s",
            nodeId, success));
    }
    
    @Override
    public void onNestedWorkflowsStart(String nodeId, int childCount) {
        logger.log(level, String.format("Node [%s] starting %d nested workflows in parallel",
            nodeId, childCount));
    }
    
    @Override
    public void onChildWorkflowComplete(String parentNodeId, int childIndex, boolean success) {
        logger.log(Level.FINE, String.format("Child workflow %d of node [%s] completed. Success: %s",
            childIndex, parentNodeId, success));
    }
    
    @Override
    public void onNestedWorkflowsComplete(String nodeId, int successCount, int failureCount) {
        logger.log(level, String.format("Node [%s] nested workflows completed: %d succeeded, %d failed",
            nodeId, successCount, failureCount));
    }
}
