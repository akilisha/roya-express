package com.akilisha.oss.roya.pipeline;
...
public class MiddlewarePipeline {
     ...
     * @param res Response object
     */
    private void executeErrorHandlersFrom(int index, Exception error, Request req, Response res) {
        if (index >= errorHandlers.size()) {
            ...
            if (!res.isHeadersSent()) {
                res.status(500).json(java.util.Map.of(
                    ...
                    "message", error.getMessage() != null ? error.getMessage() : "An error occurred"
                ));
            }
            return;
        }
