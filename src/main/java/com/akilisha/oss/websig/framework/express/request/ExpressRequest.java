...
public class ExpressRequest implements Request {
    ...
    public static Date futureDate(Duration duration) {
    ...
    public <C> C body(Class<C> bodyType) {
        try {
            Header contentType = request.getHeader("Accept");
            String mediaType = contentType.getValue().replaceAll("(^\\b.+/.+\\b)(;.*)$", "$1");
            RequestBody<C> requestBody = app.body(MimeTypes.from(mediaType));
            if (requestBody == null)
                throw new IllegalStateException("Missing 'accept' header: " + contentType.getValue());
                    ...
                    if (parts.length == 2) {
                        RequestCookie cookie = RequestCookie.create(
                                ...
                                CookieOptions.create(Map.of(
                                        ...
                                        "httpOnly", false,
                                        "domain", "localhost",
                                        "expires", futureDate(Duration.ofHours(1))
                                )));
                        cookies.add(cookie);
                    }
                }
