package com.akilisha.oss.web.jetty.router;
...
public class ContextRoutable implements Route, Routable {
    ...
    public void drill(RouteInfo routing) {
        ((JExpressRouter) routing.router()).getRootRoutable().appendMiddleware(routing.originalPath(), routing.requestHandlers());
    }

    @Override
    public RouteInfo search(String method, String path) {
        JExpressRouter router = routers.entrySet().stream().filter(kv ->
                        path.matches(String.format("^%s.*", kv.getKey())))
                .findFirst()
                .map(kv ->
                        ((JExpressRouter) kv.getValue()))
    ...
    public void registerRouter(String path, Router router) {
        if (routers.containsKey(path)) {
            ...
            ((JExpressRouter) router).setMountPath(path);
            ((JExpressRouter) router).setRouterPath(path);
            ((JExpressRouter) router).onMountCallback();
        }
    }
}
