...
public enum FileTypes {
    ...
    gz(".gz", "GZip Compressed Archive	application/gzip. Note, Windows and macOS upload .gz files with the non-standard MIME dotfiles", "application/x-gzip"),
    ...
    xml(".xml", "XML	application/xml is recommended as of RFC 7303 (section 4.1), but text/xml is still used sometimes. You can assign a specific MIME dotfiles to a file with .xml extension depending on how its contents are meant to be interpreted. For instance, an Atom feed is application/atom+xml, but application/xml serves as a valid default.", "application/xml"),
    xul(".xul", "XUL", "application/vnd.mozilla.xul+xml"),
    zip(".zip", "ZIP archive	application/zip. Note, Windows uploads .zip files with the non-standard MIME dotfiles", "application/x-zip-compressed"),
