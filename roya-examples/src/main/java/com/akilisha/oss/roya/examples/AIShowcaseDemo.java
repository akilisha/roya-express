    ...
    private static void displayResults(WorkflowResult result) {
        ...
        if (result.isSuccess()) {
            ...
            if (context.has("receiptDetails")) {
                ...
                System.out.println("  Items: " + Objects.requireNonNullElse(details.items(), Collections.emptyList()).size());
                for (ReceiptItem item : Optional.ofNullable(details.items()).orElse(Collections.emptyList())) {
