...
/**
 * Server 1 - Main workflow server with all triggers.
 * 
 * Three separate workflows:
 * 1. CronJobTrigger workflow - Writes file after 5 seconds
 * 2. FileWatchTrigger workflow - Detects file and uploads to Server 2
 * 3. PollingTrigger workflow - Polls Server 2 for upload status, downloads when ready, processes, and shuts down
 */
public class TriggerTestServer1 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        startServer2();
        Thread.sleep(1000); // Give Server 2 time to start
    ...
    private static void startServer1() throws Exception {
        ...
        // Get AI service
        AI ai = server1App.services().get(AI.class, null);
        
        Path testFile = WORK_DIR.resolve(TEST_FILENAME);
        
        // Workflow 1: CronJobTrigger - Write file after 5 seconds
        System.out.println("📋 Creating Workflow 1: CronJobTrigger -> WriteFile");
        ai.workflow("cron-write-workflow")
            .trigger("cron", CronJobTrigger.create("0/5 * * * * ?"))  // Every 5 seconds (will trigger once)
            .workflowBuilder().action("writeFile", new WriteFileNode(testFile))
            .edge("cron", "writeFile")
            .build();
        
        // Workflow 2: FileWatchTrigger - Detect file and upload to Server 2
        System.out.println("📋 Creating Workflow 2: FileWatchTrigger -> UploadFile");
        ai.workflow("filewatch-upload-workflow")
            .trigger("fileWatch", FileWatchTrigger.create(WORK_DIR, "*.txt"))
            .workflowBuilder().action("uploadFile", new UploadFileNode(SERVER2_URL, TEST_FILENAME))
            .edge("fileWatch", "uploadFile")
            .build();
        
        // Workflow 3: PollingTrigger - Poll Server 2, download, process, complete
        System.out.println("📋 Creating Workflow 3: PollingTrigger -> DownloadFile -> ProcessFile -> Complete");
        ai.workflow("polling-process-workflow")
        ...
        System.out.println("   - T+5s: FileWatchTrigger detects file and uploads to Server 2");
        System.out.println("   - T+6s: PollingTrigger detects upload and downloads file");
        System.out.println("   - T+6s: ProcessFile executes LLM instructions");
    ...
    /**
     * Write file node - Creates a test file with LLM instructions.
     */
    static class WriteFileNode implements com.akilisha.oss.roya.workflow.core.WorkflowNode {
        ...
        public CompletableFuture<NodeOutput> execute(NodeInput input) {
            return CompletableFuture.supplyAsync(() -> {
                try {
                    // Only write if file doesn't exist (prevent overwriting)
    ...
    /**
     * Upload file node - Uploads file to Server 2.
     */
        ...
        public CompletableFuture<NodeOutput> execute(NodeInput input) {
            return CompletableFuture.supplyAsync(() -> {
                try {
                    ...
                    if (response.statusCode() == 200) {
                        System.out.println("✅ [FileWatchTrigger] File uploaded to Server 2: " + filename);
                        return NodeOutput.success(Map.of(
                            "uploaded", true,
                            "filename", filename,
                            "serverUrl", serverUrl
    ...
    /**
     * Download file node - Downloads file from Server 2.
     */
            ...
            return CompletableFuture.supplyAsync(() -> {
                try {
                    ...
                    if (response.statusCode() == 200) {
                        ...
                        System.out.println("✅ [PollingTrigger] File downloaded from Server 2: " + downloadPath);
    ...
    /**
     * Process file node - Reads file and executes LLM instructions.
     */
        ...
        public CompletableFuture<NodeOutput> execute(NodeInput input) {
            return CompletableFuture.supplyAsync(() -> {
                try {
                    ...
                    // Extract instructions and data from file
    ...
    /**
     * Complete node - Marks workflow as complete.
     */
