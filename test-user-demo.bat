@echo off
REM Test User CRUD Demo with cURL
...
curl -s -X POST http://localhost:3000/users -H "Content-Type: application/json" -d "{\"name\":\"Alice\",\"email\":\"alice@example.com\",\"age\":30}"
echo.
echo.

echo 4. Updating user with id=1...
curl -s -X PUT http://localhost:3000/users/1 -H "Content-Type: application/json" -d "{\"name\":\"Alice Updated\",\"email\":\"alice.new@example.com\",\"age\":31}"
echo.
echo.
